<?php
/**
 * dashboard.php — Part 3 Updated
 * Added: Grand total at bottom of purchase history.
 * Added: Messages inbox section (messages sent by admin).
 */
...
// Fetch user data (associative read by column names)
$stmt = $conn->prepare(
...
// Fetch user's orders from tblOrderLine
$oStmt = $conn->prepare(
...
// Grand total — Part 3 requirement
$grandTotal = array_sum(array_column($orders, 'totalAmount'));

// Fetch messages sent to this user by admin
$mStmt = $conn->prepare(
    "SELECT m.subject, m.body, m.sentAt, a.fullName AS adminName
     FROM tblMessage m
     JOIN tblAdmin a ON m.fromAdminID = a.adminID
     WHERE m.toUserID = ?
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .welcome-banner { background:linear-gradient(135deg,#1a1507,#2a1e0a); border:1px solid #3a2d10; border-radius:var(--radius); padding:1.5rem 2rem; margin-bottom:2rem; }
  ...
  .info-table th { background:#1f1f1f; padding:.7rem 1rem; text-align:left; font-size:.8rem; color:var(--muted); text-transform:uppercase; letter-spacing:.05em; border-bottom:1px solid var(--border); }
  .info-table td { padding:.7rem 1rem; border-bottom:1px solid var(--border); font-size:.92rem; }
  .info-table tr:last-child td { border-bottom:none; }
  .orders-table { width:100%; border-collapse:collapse; margin-bottom:2.5rem; }
  .orders-table th { background:#1f1f1f; padding:.7rem 1rem; font-size:.8rem; color:var(--muted); text-transform:uppercase; text-align:left; border-bottom:1px solid var(--border); }
  .orders-table td { padding:.65rem 1rem; border-bottom:1px solid var(--border); font-size:.9rem; }
  .orders-table .grand-total-row td { border-top:2px solid var(--gold); font-weight:700; color:var(--gold); background:#1a1a0a; }
  ...
  .btn-logout { background:transparent; border:1px solid var(--border); color:var(--muted); padding:.4rem .9rem; border-radius:var(--radius); cursor:pointer; font-size:.85rem; text-decoration:none; }
  .btn-logout:hover { border-color:#e05252; color:#e05252; }

  /* Messages */
  .message-card { background:var(--card); border:1px solid var(--border); border-radius:var(--radius); padding:1rem 1.25rem; margin-bottom:.75rem; }
  .message-card .msg-from { font-size:.75rem; color:var(--muted); margin-bottom:.2rem; }
  .message-card .msg-subject { font-weight:600; color:var(--gold); margin-bottom:.4rem; }
  .message-card .msg-body { font-size:.9rem; color:var(--text); }
  .message-card .msg-date { font-size:.75rem; color:var(--muted); margin-top:.4rem; }
</style>
</head>
<body>
<nav>
...
<div class="container">
  ...
  <table class="info-table">
    ...
    <tr>
      ...
      <td>
        <?php $s = $user['status']; $cls = $s==='active'?'badge-active':($s==='pending'?'badge-pending':'badge-inactive'); ?>
  ...
  <!-- Seller shortcut -->
  <?php if (($_SESSION['role'] ?? null) === 'seller'): ?>
  <div style="background:linear-gradient(135deg,#1a1507,#2a1e0a);border:1px solid #3a2d10;border-radius:var(--radius);padding:1.25rem 2rem;margin-bottom:2rem;display:flex;align-items:center;justify-content:space-between;">
    <div>
      <div style="font-family:'Playfair Display',serif;color:var(--gold);font-size:1rem;margin-bottom:.25rem;">🏷️ Seller Dashboard</div>
      <div style="color:var(--muted);font-size:.85rem;">Manage your listings, upload new items and track your sales.</div>
    </div>
    <a href="seller-products.php" style="background:var(--gold);color:#111;padding:.6rem 1.5rem;border-radius:var(--radius);text-decoration:none;font-weight:600;font-size:.9rem;white-space:nowrap;">My Products →</a>
  </div>
  <?php endif; ?>

  <!-- Purchase History with Grand Total — Part 3 requirement -->
  ...
  <?php else: ?>
    <table class="orders-table">
      ...
      <?php endforeach; ?>
      <!-- Grand Total row — Part 3 requirement -->
      <tr class="grand-total-row">
        <td colspan="4" style="text-align:right;">Grand Total of All Purchases:</td>
  ...
  <!-- Messages Inbox — Part 3 requirement -->
  <div class="section-title">✉️ Messages from Admin</div>
  <?php if (empty($messages)): ?>
    <p class="empty">No messages yet.</p>
  <?php else: ?>
    <?php foreach ($messages as $m): ?>
      <div class="message-card">
        <div class="msg-from">From: <?= htmlspecialchars($m['adminName']) ?> (Admin)</div>
        <div class="msg-subject"><?= htmlspecialchars($m['subject']) ?></div>
        <div class="msg-body"><?= nl2br(htmlspecialchars($m['body'])) ?></div>
        <div class="msg-date"><?= date('d M Y H:i', strtotime($m['sentAt'])) ?></div>
      </div>
    <?php endforeach; ?>
  <?php endif; ?>
