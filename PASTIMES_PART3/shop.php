<?php
/**
 * shop.php — Part 3 Updated
 * Now uses the ShoppingCart OOP class.
 * Added: cart quantity edit, ShoppingCart class integration.
 */
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .btn-showcart { background:transparent; border:1px solid var(--gold); color:var(--gold); padding:.35rem .9rem; border-radius:var(--radius); font-size:.82rem; text-decoration:none; font-weight:500; margin-left:.5rem; display:inline-block; }
  .btn-showcart:hover { background:var(--gold); color:#111; }
  .cart-badge { background:var(--gold); color:#111; font-size:.7rem; font-weight:700; border-radius:50%; padding:.15rem .4rem; margin-left:.3rem; }
  ...
  .product-card { background:var(--card); border:1px solid var(--border); border-radius:var(--radius); overflow:hidden; transition:transform .2s, border-color .2s; }
  .product-card:hover { transform:translateY(-3px); border-color:#3a3a3a; }
  .product-img { width:100%; height:200px; object-fit:cover; background:#1f1f1f; display:flex; align-items:center; justify-content:center; color:var(--muted); font-size:.8rem; }
  ...
  .cond-badge { display:inline-block; font-size:.7rem; padding:.15rem .5rem; border-radius:20px; font-weight:600; margin-bottom:.6rem; color:#fff; }
  ...
  .btn-cart { width:100%; padding:.55rem; background:var(--gold); color:#111; border:none; border-radius:var(--radius); cursor:pointer; font-weight:600; font-size:.9rem; display:flex; align-items:center; justify-content:center; gap:.4rem; }
  .btn-cart:hover { background:#e8c98a; }

  /* Price popup */
  .price-popup { display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:#1a1a1a; border:1px solid var(--gold); border-radius:var(--radius); padding:2rem; z-index:999; min-width:280px; text-align:center; box-shadow:0 20px 60px rgba(0,0,0,.8); }
  ...
  /* Cart sidebar */
  .cart-panel { background:var(--card); border:1px solid var(--border); border-radius:var(--radius); padding:1.25rem; position:sticky; top:1rem; max-height:80vh; overflow-y:auto; }
  .cart-panel h3 { font-family:'Playfair Display',serif; color:var(--gold); margin-bottom:1rem; font-size:1.1rem; }
  .cart-item { padding:.65rem 0; border-bottom:1px solid var(--border); font-size:.88rem; }
  .cart-item-title { margin-bottom:.3rem; font-weight:500; }
  .cart-item-price { color:var(--gold); font-weight:600; margin-top:.2rem; }
  ...
  .checkout-btn { width:100%; margin-top:1rem; padding:.65rem; background:var(--gold); color:#111; border:none; border-radius:var(--radius); cursor:pointer; font-weight:700; font-size:.95rem; display:block; text-align:center; text-decoration:none; }
  .empty-cart { color:var(--muted); font-size:.88rem; text-align:center; padding:1rem 0; }

  /* Cart qty edit */
  .qty-form { display:flex; align-items:center; gap:.4rem; margin:.3rem 0; }
  .qty-form input[type=number] { width:52px; padding:.25rem .4rem; background:#1f1f1f; border:1px solid var(--border); color:var(--text); border-radius:4px; font-size:.82rem; text-align:center; }
  .qty-label { font-size:.75rem; color:var(--muted); }
...
<nav>
  ...
  <div class="nav-links">
    ...
    <?php endif; ?>
    <a href="shop.php">🛒<?php if ($cartItems): ?><span class="cart-badge"><?= count($cartItems) ?></span><?php endif; ?></a>
    <?php if (!empty($cartItems)): ?>
      <a href="#cart-section" class="btn-showcart" id="showCartBtn">ShowCart (<?= count($cartItems) ?>)</a>
    <?php endif; ?>
  </div>
</nav>
...
<div class="layout">
  <div>
    ...
    <div class="grid">
      <?php foreach ($clothes as $item): ?>
        <?php
          $cc      = $condColor[$item['condition_']] ?? '#888';
        ...
        <div class="product-card">
          ...
          <div class="product-body">
            <div class="cond-badge" style="background:<?= $cc ?>">
  ...
  <!-- Cart sidebar -->
  <div class="cart-panel" id="cart-section">
    <h3>🛒 Cart (<?= count($cartItems) ?>)</h3>
    <?php if (empty($cartItems)): ?>
      <div class="empty-cart">Your cart is empty</div>
    <?php else: ?>
      <?php foreach ($cartItems as $ci): ?>
        <div class="cart-item">
          <div class="cart-item-title"><?= htmlspecialchars($ci['title']) ?></div>
          <!-- Quantity edit form — Part 3 requirement -->
          <form method="POST" action="shop.php" class="qty-form">
            <input type="hidden" name="updateID" value="<?= $ci['clothesID'] ?>">
            <span class="qty-label">Qty:</span>
            <input type="number" name="newQty"
                   value="<?= $ci['qty'] ?>" min="1" max="20"
